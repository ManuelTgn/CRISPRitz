...
{
    record("complement '?' passthrough", NucleotideEncoder::complement('?') == '?');
    record("complement 'N' passthrough", NucleotideEncoder::complement('N') == 'N');
}
...
{
    // ACGT is self-complementary when reversed: rc(ACGT) = rc(T)rc(G)rc(C)rc(A) = ACGT
