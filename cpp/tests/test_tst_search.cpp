...
static void test_exact_match_absent_guide() {
  ...
  LoadedTST tst = load_partition(path);
  TSTSearcher s(cfg(0));
  auto hits = s.search(tst, "TTTTTTTT"); // completely different
...
static void test_one_mismatch_found() {
  ...
  {
    TSTSearcher s(cfg(0));
    auto hits = s.search(tst, "AGGTACGT");
...
static void test_two_mismatch_budget() {
  ...
  {
    TSTSearcher s(cfg(1));
    auto hits = s.search(tst, "AGCTACGT");
...
static void test_no_results_empty_query_set() {
  ...
  LoadedTST tst = load_partition(path);
  TSTSearcher s(cfg(2));
  SearchResult r = s.search_all(tst, {});
...
static void test_multi_site_leaf_chain() {
  ...
  LoadedTST tst = load_partition(path);
  TSTSearcher s(cfg(0));
  auto hits = s.search(tst, "ACGTACGT");
...
static void test_search_partition_end_to_end() {
  ...
  SearchResult r = search_partition(path, {"ACGTACGT", "TTTTTTTT"}, cfg(0));
