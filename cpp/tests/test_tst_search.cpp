...
namespace fixture
{
    ...
    struct Builder
    {
        ...
        int insert(const std::string& guide, int guide_index, const std::string& pam)
        {
        ...
        {
            if (c == '0') return NULL_CHILD_NIBBLE;
            if (c == '_') return SENTINEL_NIBBLE;
        ...
        void serialize_node(int node_idx, Writer& w) const
        {
            ...
            if (node.lokid > 0) serialize_node(node.lokid, w);
            else                w.buffer_char('0');

            if (node.hikid > 0) serialize_node(node.hikid, w);
            else                w.buffer_char('0');
            ...
            {
                w.buffer_char('_');
                if (w.buf_pos == 1) { w.buf[1] = '0'; w.buf_pos = 2; w.flush_pair(); }
    ...
    std::string make_bin(const std::string& dir,
...
static SearchConfiguration cfg(int mm, int bdna = 0, int brna = 0, int threads = 1)
{
...
static void test_exact_match_found()
{
    ...
    auto path = fixture::make_bin("/tmp", "NGG", "chrEx", 8,
                                  {{guide, 100, "AGG"}});
...
static void test_exact_match_absent_guide()
{
    ...
    auto path = fixture::make_bin("/tmp", "NGG", "chrNo", 8,
                                  {{"ACGTACGT", 100, "AGG"}});
    LoadedTST tst = load_partition(path);
    TSTSearcher s(cfg(0));
...
static void test_one_mismatch_found()
{
    ...
    auto path = fixture::make_bin("/tmp", "NGG", "chrMm", 8,
                                  {{"ACGTACGT", 200, "AGG"}});
    LoadedTST tst = load_partition(path);
    ...
    // mm=1 SHOULD find it
    {
        ...
        for (const auto& h : hits)
            if (h.mismatches() == 1 && !h.has_bulge()) found_one_mm = true;
...
static void test_two_mismatch_budget()
{
    ...
    auto path = fixture::make_bin("/tmp", "NGG", "chrMm2", 8,
                                  {{"ACGTACGT", 300, "AGG"}});
    LoadedTST tst = load_partition(path);
    ...
    }
    {
        ...
        auto hits = s.search(tst, "AGCTACGT", "chrTest");
        bool found = false;
        for (const auto& h : hits) if (h.mismatches() == 2) found = true;
...
static void test_multi_site_leaf_chain()
{
    ...
    auto path = fixture::make_bin("/tmp", "NGG", "chrChain", 8,
                                  {{"ACGTACGT", 100, "AGG"},
                                   {"ACGTACGT", 555, "CGG"}});
    LoadedTST tst = load_partition(path);
    TSTSearcher s(cfg(0));
...
static void test_reverse_strand_index()
{
    ...
    for (const auto& h : hits) if (h.strand() == Strand::Reverse) any_reverse = true;
...
static void test_dna_bulge()
{
    ...
    bool threw = false;
    std::size_t n = 0;
    try { auto hits = s.search(tst, "ACGTACGT", "chrTest"); n = hits.size(); }
    catch (...) { threw = true; }
...
static void test_edge_budget_exceeds_guide_length()
{
    ...
    bool threw = false;
    try { (void)s.search(tst, "ACGTACGT", "chrTest"); }
...
{
    auto path = fixture::make_bin("/tmp", "NGG", "chrLen", 8,
                                  {{"ACGTACGT", 100, "AGG"}});
    LoadedTST tst = load_partition(path);   // guide_length == 8
    TSTSearcher s(cfg(0));
    bool threw = false;
    try { (void)s.search(tst, "ACGT", "chrTest"); }    // length 4 != 8
    catch (const std::invalid_argument&) { threw = true; }
...
{
    auto path = fixture::make_bin("/tmp", "GG", "chrOne", 1,
                                  {{"A", 100, "GG"}});
    LoadedTST tst = load_partition(path);
...
static void test_search_partition_end_to_end()
{
    ...
    auto path = fixture::make_bin("/tmp", "NGG", "chrE2E", 8,
                                  {{"ACGTACGT", 100, "AGG"}});
    SearchResult r = search_partition(path, "chrE2E", {"ACGTACGT", "TTTTTTTT"}, cfg(0));
    ...
    record("search_partition: source_path preserved",
           r.source_path == path);
    std::remove(path.c_str());
}
