 ...
 *   - empty output      : header-only when there are no records; header suppressed
...
{
    return OffTarget{chrom, pos, strand,
...
static std::vector<std::string> fields(const std::string& line, char sep = '\t')
{
    std::vector<std::string> out;
    std::string cur;
    for (char c : line)
...
{
    bool threw = false;
    try { (void)make_formatter(static_cast<OutputFormat>(99)); }
...
{
    bool threw = false;
    try { OutputWriter w{std::unique_ptr<OffTargetFormatter>(nullptr)}; }
    catch (const std::invalid_argument&) { threw = true; }
...
static void test_malformed_path_throws()
{
    ...
    bool threw = false;
    try { (void)w.write_to_file({hit("chr1", 1, Strand::Forward)}, bad); }
...
static void test_malformed_path_searchresult_throws()
{
    ...
    bool threw = false;
    try { (void)w.write_to_file(result, bad); }
    catch (const std::runtime_error&) { threw = true; }
