...
public:
   ...
   *
   * @note Interface only — the traversal algorithm is implemented in a
   *       later phase.
   */
  [[nodiscard]] std::vector<OffTarget> search(const LoadedTST &tst,
                                              std::string_view guide_seq) const;
  ...
  search_all(const LoadedTST &tst,
             const std::vector<std::string> &guides) const;
 ...
 *                               loaded guide length.
 *
 * @note Interface only — implemented in a later phase.
 */
[[nodiscard]] SearchResult
search_partition(const std::string &partition_path,
