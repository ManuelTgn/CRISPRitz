...
namespace crispritz
{
    ...
    class LoadedTST
    {
      public:
         ...
         *         @p guide_length <= 0.
         */
        LoadedTST(std::vector<TSTNode> nodes,
                  std::vector<TSTLeaf> leaves,
                  int                  guide_length,
                  std::string          source_path);

        /** @return The TST node pool (root at index 0). */
        [[nodiscard]] const std::vector<TSTNode>& nodes() const noexcept { return nodes_; }

        /** @return The leaf array. */
        [[nodiscard]] const std::vector<TSTLeaf>& leaves() const noexcept { return leaves_; }
    ...
    class TSTSearcher
    {
      public:
        ...
        [[nodiscard]] const SearchConfiguration& config() const noexcept { return config_; }
     ...
     *                               loaded guide length.
     */
    [[nodiscard]] SearchResult search_partition(const std::string&              partition_path,
