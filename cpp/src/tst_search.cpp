...
namespace crispritz
{
    ...
    LoadedTST::LoadedTST(std::vector<TSTNode> nodes,
                         std::vector<TSTLeaf> leaves,
                         int                  guide_length,
                         std::string          source_path)
        : nodes_(std::move(nodes))
        , leaves_(std::move(leaves))
        , guide_length_(guide_length)
        , source_path_(std::move(source_path))
    {
        ...
        if (guide_length_ <= 0)
            throw std::invalid_argument(
                "LoadedTST: guide_length must be > 0, got " +
    ...
    namespace
    {
        ...
        void rebuild_children(NibbleReader& r, std::vector<TSTNode>& nodes, int self_idx);
    ...
    LoadedTST load_partition(const std::string& partition_path)
    {
        ...
        if (buf.size() < 8u)
            throw std::runtime_error("load_partition: file too small: " + partition_path);
        ...
        if (chunk_size < 0 || guide_length <= 0)
            throw std::runtime_error("load_partition: invalid header in " + partition_path);
        ...
        if (pam_bytes < 0)
            throw std::runtime_error(
                "load_partition: cannot derive PAM width from filename: " + partition_path);
        ...
        if (node_count < 0)
            throw std::runtime_error("load_partition: negative node_count in " + partition_path);
        ...
        return LoadedTST{std::move(nodes), std::move(leaves), guide_length, partition_path};
    }
    ...
    namespace
    {
                             ...
                             std::vector<OffTarget>& out)
        {
            ...
            while (idx >= 0 && idx < static_cast<int>(leaves.size()))
            {
                ...
                const int     leftmost0 = leaf.guide_index >= 0 ? leaf.guide_index
                                                                : -leaf.guide_index;
                ...
                for (std::size_t k = 0; k < frame.aln_target.size(); ++k)
                {
                    ...
                    if (gd == '-')      ++bdna_count;       // gap in guide  = DNA bulge
                    else if (tg == '-') ++brna_count;       // gap in target = RNA bulge
                    else if (tg >= 'a' && tg <= 'z') ++mm_count; // lowercase = mismatch
                }
                      ...
                      std::vector<OffTarget>& out)
        {
            ...
            const bool is_root_sentinel =
                (node_idx == 0 && node.splitchar_enc == 0 &&
                                               ...
                                               const std::string& chrom) const
    {
        ...
        if (config_.max_total_edits() > tst.guide_length())
            throw std::invalid_argument(
                "TSTSearcher::search: max_total_edits (" +
        ...
        if (static_cast<int>(guide_seq.size()) != tst.guide_length())
            throw std::invalid_argument(
                "TSTSearcher::search: guide length (" +
        ...
        root.guide_pos = 0;
        root.aln_guide.reserve(static_cast<std::size_t>(ctx.guide_len + config_.max_bulges_total()));
        root.aln_target.reserve(static_cast<std::size_t>(ctx.guide_len + config_.max_bulges_total()));
