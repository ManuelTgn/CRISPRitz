...
namespace pam
{
    ...
    CompactGenome::CompactGenome(std::string_view sequence) : length_(sequence.length())
    {
    ...
    std::vector<int> search_pam_sites_fast(std::string_view pam_sequence,
                                           const CompactGenome& genome, const SearchParams& params)
    {
...
#pragma omp for schedule(static) nowait
                for (size_t pos = 0; pos < search_end; ++pos)
                {
                    ...
                    {
                        int guide_start = static_cast<int>(pos + pam_limit - 1) - (pam_len - 1);
                        if (guide_start >= 0)
                        {
...
#pragma omp critical
    ...
    std::vector<int> search_pam_sites(std::string_view pam_sequence,
                                      std::string_view genome_sequence, const SearchParams& params)
    {
