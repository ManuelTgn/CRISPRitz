...
def retrieve_contig_names(fasta_files: List[str], verbosity: int,  debug: bool) -> Set[str]:
    # retrieve contig names for each fasta file in genome folder
    print_verbosity("Retrieving contig names from FASTA in genome folder", verbosity, VERBOSITYLVL[3])
    return {_retrieve_contig_name(FastaFile(f), debug) for f in fasta_files}

def initialize_fasta(fasta_vcf_map: Dict[str, EnrichPair], fasta_files: List[str], debug: bool) -> Dict[str, EnrichPair]:
    for f in fasta_files:
        ...
        if fasta_vcf_map[contig].fasta is not None:  # multiple fasta pointing to same contig
            exception_handler(
                CrispritzEnrichmentError,
...
def initialize_vcf(fasta_vcf_map: Dict[str, EnrichPair], vcf_files: List[str], debug: bool) -> Dict[str, EnrichPair]:
    for f in vcf_files:
        contig = _retrieve_contig_vcf(VariantFile(f, mode="r"), debug)  # retrieve vcf contig
        if fasta_vcf_map[contig].vcf is not None:  # multiple vcf pointing to same contig
            exception_handler(
                CrispritzEnrichmentError,
...
def construct_fasta_vcf_map(fasta_files: List[str], vcf_files: List[str], verbosity: int, debug: bool) -> Dict[str, EnrichPair]:
    # retrieve contig names in fasta from genome folder and initialize map
    fasta_vcf_map = {contig: EnrichPair(debug) for contig in retrieve_contig_names(fasta_files, verbosity, debug)}
...
def prepare_output_dir(outdir: str) -> Tuple[str, str]:
    return create_folder(os.path.join(outdir, SNPDIR)), create_folder(os.path.join(outdir, INDELSDIR))

def enrich_no_variants(fasta_vcf_map: Dict[str, EnrichPair], contigs: List[str], outdir: str, debug: bool) -> None:
...
def add_variants(args: CrispritzEnrichmentInputArgs) -> None:
    ...
    fasta_vcf_map = construct_fasta_vcf_map(args.fastas, args.vcfs, args.verbosity, args.debug)
    start = time()  # genome enrichment start point
    print_verbosity("Enriching genome with input variants", args.verbosity, VERBOSITYLVL[1])
